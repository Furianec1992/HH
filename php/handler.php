		...
		if (isset($_REQUEST['savebtn'])) {
	  	foreach($_REQUEST as $key=>$value){
			...
			//проверка закрытого месяца
		          $MounthDateQuery = $mysqli->query("SELECT * FROM timesheet WHERE timesheet_id='" . $hrs_id . "'");
	  		  $MounthDateRow = $MounthDateQuery->fetch_array(MYSQLI_ASSOC);
	  		  
			  $MounthDate = date('Y-m-01', strtotime($date));
	  		  $CalendarMounthDateQuery = $mysqli->query("SELECT MounthDate_closets FROM MounthDate WHERE MounthDate_date='" . $MounthDate . "'");
	  		  $CalendarMounthDateClose = $CalendarMounthDateQuery->fetch_array(MYSQLI_ASSOC);
	  		  //echo($CalendarMounthDateClose['MounthDate_closets']*1); 
	  		  
	  		  // Если месяц закрыт, прерываем сохранение
			 if ($CalendarMounthDateClose['MounthDate_closets'] == 1 and $MounthDateRow['timesheet_hours'] != $value) {
    				echo "<script> alert('Сохранение прервано (0). На дату " . date('d.m.Y', strtotime($date)) . "г. отчетный месяц закрыт.'); </script>";
			...
			//Проверяем есть ли остаток бюджета
			// Получаем общее количество потраченных часов по проекту
			$queryTotalHours = $mysqli->query("SELECT SUM(timesheet_hours) AS total_spent_hours FROM timesheet WHERE project_id='" . $project_id . "' AND timesheet_del=0");
			$rowTotalHours = $queryTotalHours->fetch_array(MYSQLI_ASSOC);

			// Получаем общий бюджет часов по контракту и название проекта
			$queryBudgetInfo = $mysqli->query("SELECT Hours AS budget_hours, project_nameRusShort FROM lst_project  WHERE project_id='" . $project_id . "'");
			$rowBudgetInfo = $queryBudgetInfo->fetch_array(MYSQLI_ASSOC);
			
			// Вычисляем остаток бюджета
			$remainingBudget = $rowBudgetInfo['budget_hours'] - ($rowTotalHours['total_spent_hours'] - $row['timesheet_hours'] + $value);

			// Если остаток отрицателен, прерываем сохранение
			if ($remainingBudget < 0 && $value > 0) {
    				echo "<script> alert('Сохранение прервано (1). Остаток бюджета по этапу проекта \"" . htmlspecialchars($rowBudgetInfo['project_nameRusShort'], ENT_QUOTES) . "\" исчерпан.'); </script>";
				echo "<script> location ='/php/doc/timesheet.php?doc_id=". $_REQUEST['00-doc_id'] . "';</script>";
				exit();
			}
			
			// Проверяем, не превысила ли введённая дата конец проекта
			$queryCheck = $mysqli->query("SELECT EndDatePlan, project_nameRusShort FROM lst_project WHERE project_id='" . $project_id . "'");
			$rowCheck = $queryCheck->fetch_array(MYSQLI_ASSOC);

			if(strtotime($date) > strtotime($rowCheck['EndDatePlan'])){
				$formattedDate = date('d.m.Y', strtotime($date));
   				$formattedEndDate = date('d.m.Y', strtotime($rowCheck['EndDatePlan']));
   				echo "<script> alert('Сохранение прервано (2). Дата в табеле ($formattedDate г.) превышает дату окончания проекта ($formattedEndDate г.) - " . htmlspecialchars($rowCheck['project_nameRusShort'], ENT_QUOTES) . "'); </script>";
   				echo "<script> location ='/php/doc/timesheet.php?doc_id=". $_REQUEST['00-doc_id'] . "';</script>";
				exit();
			}
			...
			// Вычисляем остаток бюджета
			$remainingBudget = $rowBudgetInfo['budget_hours'] - $rowTotalHours['total_spent_hours'] - $value;

			// Если остаток отрицателен, прерываем сохранение
			if ($remainingBudget < 0 && $value > 0) {
    				echo "<script> alert('Сохранение прервано (3). Остаток бюджета по этапу проекта \"" . htmlspecialchars($rowBudgetInfo['project_nameRusShort'], ENT_QUOTES) . "\" исчерпан.'); </script>";
				echo "<script> location ='/php/doc/timesheet.php?doc_id=". $_REQUEST['00-doc_id'] . "';</script>";
				exit();
			}
			...
			if(strtotime($date) > strtotime($rowCheck['EndDatePlan'])){
				$formattedDate = date('d.m.Y', strtotime($date));
   				$formattedEndDate = date('d.m.Y', strtotime($rowCheck['EndDatePlan']));
   				echo "<script> alert('Сохранение прервано (4). Дата в табеле ($formattedDate г.) превышает дату окончания проекта ($formattedEndDate г.) - " . htmlspecialchars($rowCheck['project_nameRusShort'], ENT_QUOTES) . "'); </script>";
   				echo "<script> location ='/php/doc/timesheet.php?doc_id=". $_REQUEST['00-doc_id'] . "';</script>";
				exit();
			}
