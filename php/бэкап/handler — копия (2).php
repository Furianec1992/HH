  ...
  if ($_REQUEST['site'] == "timesheet.php"){
		if (isset($_REQUEST['savebtn'])) {
	  	foreach($_REQUEST as $key=>$value){
			...
			// Получаем общий бюджет часов по контракту
			$queryBudgetHours = $mysqli->query("SELECT Hours AS budget_hours FROM contract_project_names WHERE project_id='" . $project_id . "'");
			$rowBudgetHours = $queryBudgetHours->fetch_array(MYSQLI_ASSOC);

			// Получаем общий бюджет часов по контракту и название проекта
			$queryBudgetInfo = $mysqli->query("SELECT Hours AS budget_hours, project_nameRusShort FROM contract_project_names WHERE project_id='" . $project_id . "'");
			$rowBudgetInfo = $queryBudgetInfo->fetch_array(MYSQLI_ASSOC);
			
			// Вычисляем остаток бюджета
			$remainingBudget = $rowBudgetHours['budget_hours'] - ($rowTotalHours['total_spent_hours'] - $row['timesheet_hours'] + $value);
			...
			// Вычисляем остаток бюджета
			$remainingBudget = $rowBudgetHours['budget_hours'] - $rowTotalHours['total_spent_hours'] - $value;
