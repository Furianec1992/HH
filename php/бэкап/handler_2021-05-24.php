		      ...
		      $query = $mysqli->query("SELECT * FROM timesheet WHERE timesheet_id='" . $hrs_id . "'");
	  		  ...
	  		  if ($row['wedate_closets'] == 0) {
    	  		if ($nextkeyUPDATE) {
    	  		  ...
    	  		  $query = $mysqli->query("INSERT INTO timesheet (timesheet_date, project_id, hours_type_id, timesheet_hours, activity_id, timesheet_dateCorr, doc_id) 
    	  		                           VALUES ('" . $date . "', '" . $project_id . "', '" . $hourstype_id . "', '" . $value . "', '" .$activity_id . "', '" . $dateCorr . "', '" . $doc_id . "')");
