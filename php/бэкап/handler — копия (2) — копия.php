		...
		if (isset($_REQUEST['savebtn'])) {
	  	foreach($_REQUEST as $key=>$value){
			...
			if ($remainingBudget < 0) {
    				echo "<script> alert('Сохранение прервано. Остаток бюджета по этапу проекта \"" . htmlspecialchars($rowBudgetInfo['project_nameRusShort'], ENT_QUOTES) . "\" исчерпан.'); </script>";
			...
			if(strtotime($date) > strtotime($rowCheck['EndDatePlan'])){
				$formattedDate = date('d.m.Y', strtotime($date));
   				...
   				echo "<script> alert('Сохранение прервано. Дата в табеле ($formattedDate г.) превышает дату окончания проекта ($formattedEndDate г.) - " . htmlspecialchars($rowCheck['project_nameRusShort'], ENT_QUOTES) . "'); </script>";
   				echo "<script> location ='/php/doc/timesheet.php?doc_id=". $_REQUEST['00-doc_id'] . "';</script>";
				exit();
			}
