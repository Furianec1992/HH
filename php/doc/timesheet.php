...
// заполнение списков
	...
	$queryTotalHours = $mysqli->query("SELECT SUM(timesheet_hours) AS total_spent_hours FROM timesheet WHERE project_id='" . $row['project_id'] . "' AND timesheet_del=0");
	$rowTotalHours = $queryTotalHours->fetch_array(MYSQLI_ASSOC);

	// Получаем общий бюджет часов по контракту и название проекта
	$queryBudgetInfo = $mysqli->query("SELECT Hours AS budget_hours, project_nameRusShort FROM contract_project_names WHERE project_id='" . $row['project_id'] . "'");
	$rowBudgetInfo = $queryBudgetInfo->fetch_array(MYSQLI_ASSOC);

	// Вычисляем остаток бюджета
	$remainingBudget = $rowBudgetInfo['budget_hours'] - $rowTotalHours['total_spent_hours'];

	    $disabled = ($remainingBudget < 1) ? " disabled" : "";
    
    	// Форматирование часов
	    $hours = $row['Hours'];
	    $formattedHours = (floor($remainingBudget) == $remainingBudget) ? (int)$remainingBudget : number_format($remainingBudget, 2, '.', '');
	    $formatedEndDate = date('d.m.Y', strtotime($row['EndDatePlan']));
    
	    $project .= "<option value=\"" . $row['project_id'] . "\"" . $disabled . ">" 
        	     . $row['project_nameEngShort'] // Название проекта
	             //. " [Остаток часов: $formattedHours ч.] " // Остаток часов сразу после названия
		     . " [Дата окончания работ: " . $formatedEndDate ."г.]" // Остаток часов сразу после названия
...
// заполнение часов в табеле!!!!!!!!!!!!!
    ...
    while ($row = $query->fetch_array(MYSQLI_ASSOC)) {
      ...
      //определение закрыт ли месяц
      $subquery = $mysqli->query("SELECT * FROM MounthDate WHERE MounthDate_date='" . date('Y-m-01', strtotime($row['timesheet_date'])) . "'");
      while ($wedate = $subquery->fetch_array(MYSQLI_ASSOC)){
        if ($wedate['MounthDate_closets']) {$atrrHrsDisabled = "disabled";} else {$atrrHrsDisabled = "";}
      }
//echo "<script> alert('Сохранение прервано (0). На дату " . date('Y-m-01', strtotime($row['timesheet_date'])) . "г. отчетный месяц закрыт.'); </script>";
      //if (!$subquery->num_rows) {$atrrHrsDisabled = "disabled";};
	...
	<div class="mdl-card mdl-cell mdl-cell--12-col">
		...
		<div class="mdl-card__supporting-text">
			...
			<div class="mdl-card__actions mdl-card--border">
				...
				<a class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect" href="http://directumrx.sngt.ru/Sungero?type=09584896-81e2-4c83-8f6c-70eb8321e1d0&id=342384"
				  >Как заполнять?</a>
			</div>
			</div>
	</div>
