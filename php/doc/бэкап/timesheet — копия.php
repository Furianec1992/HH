...
// заполнение списков
    ...
    while ($row = $query->fetch_array(MYSQLI_ASSOC)) {$project = $project . "<option value=" . $row['project_id'] . ">" . $row['project_nameEngShort'] . " (Дог." . $row['contract_number'] . ")" . "</option>";}
      ...
      if (!$subquery->num_rows) {$atrrHrsDisabled = "disabled";};
    	...
    	if ($hrsDescIndex != $hrsDescIndex_new || empty($hrsDescIndex)) {
    		...
    		$hrsDescHTML = $hrsDescHTML . "<tr class='mainrows'><td> " . $r=$r+1 . "</td><td style='white-space: normal;' class='mdl-data-table__cell--non-numeric'>" .
    		  substr($row["project_nameEngShort"],0,80) . " (Дог." . $row["contract_number"] . ")" . "</td>
