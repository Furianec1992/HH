...
// заполнение списков
    ...
    while ($row = $query->fetch_array(MYSQLI_ASSOC)) {$project = $project . "<option value=" . $row['project_id'] . ">" . $row['contract_number'] . " - ". $row['project_nameEngShort'] . "</option>";}
    ...
    while ($row = $query->fetch_array(MYSQLI_ASSOC)) {
      ...
      if (!$subquery->num_rows) {$atrrHrsDisabled = "disabled";};
    	...
    	if ($hrsDescIndex != $hrsDescIndex_new || empty($hrsDescIndex)) {
    		...
    		$hrsDescHTML = $hrsDescHTML . "<tr class='mainrows'><td> " . $r=$r+1 . "</td><td style='white-space: normal;' class='mdl-data-table__cell--non-numeric'>" . $row["contract_number"] . " - " .
    		  substr($row["project_nameEngShort"],0,80) . "</td>
		...
		<div class="mdl-card__supporting-text">
			<form id="timesheetForm" method="post" action="/php/handler.php" >
	          ...
	          <tr class="mainrows">
							...
							<th class="mdl-data-table__cell--non-numeric" style="width: 663px !important; vertical-align: middle; text-align: center;">Контракт - Проект</th>
